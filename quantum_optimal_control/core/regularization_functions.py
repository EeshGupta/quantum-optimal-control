...
def get_reg_loss_tf2(tfs, ops_weight):
    ...
    with tf.name_scope('reg_errors'):
        ...
        if 'amplitude' in tfs.sys_para.reg_coeffs:
            ...
            reg_loss = reg_loss + amp_reg_alpha * tf.nn.l2_loss(ops_weight)
        ...
        if 'envelope' in tfs.sys_para.reg_coeffs:
            ...
            reg_loss = reg_loss + reg_alpha * tf.nn.l2_loss(
                tf.multiply(tfs.tf_one_minus_gaussian_envelope, ops_weight))
        ...
        if 'dwdt' in tfs.sys_para.reg_coeffs:
           ...
           # print('debug')
            ...
            new_weights = tf.concat([ops_weight, zeros_for_training], 1)
        ...
        if 'bandpass' in tfs.sys_para.reg_coeffs:
            ...
            tf_u = tf.cast(ops_weight, dtype=tf.complex64)
        ...
        if 'forbidden_coeff_list' in tfs.sys_para.reg_coeffs:
            print('no support for forbidden coeff list yet')
        ...
        if 'speed_up' in tfs.sys_para.reg_coeffs:
            print('no support for speed_up currently')
